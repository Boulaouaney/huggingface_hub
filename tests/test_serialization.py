...
def _get_safetensors_metadata(safetensors_path: Path) -> dict:
    file_bytes = safetensors_path.read_bytes()
    metadata_len = struct.unpack("<Q", file_bytes[:8])[0]
    metadata_str = file_bytes[8 : 8 + metadata_len].decode()
...
def test_save_torch_state_dict_shared_layers_not_sharded(
    ...
    metadata = _get_safetensors_metadata(safetensors_file)
